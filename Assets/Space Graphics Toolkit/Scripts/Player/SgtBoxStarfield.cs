...
public class SgtBoxStarfield : SgtStarfield
{
	...
	public static SgtBoxStarfield CreateBoxStarfield(Transform parent = null)
	{
		return CreateBoxStarfield(parent, Vector3.zero, Quaternion.identity, Vector3.one);
	}
	
	public static SgtBoxStarfield CreateBoxStarfield(Transform parent, Vector3 localPosition, Quaternion localRotation, Vector3 localScale)
	{
		var gameObject = SgtHelper.CreateGameObject("Box Starfield", parent, localPosition, localRotation, localScale);
	...
	{
		var starfield = CreateBoxStarfield(SgtHelper.GetSelectedParent());
