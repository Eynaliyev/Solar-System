...
public class SgtLightning : MonoBehaviour
{
	...
	public static SgtLightning Create(Transform parent, float size, float radius, float detail)
	{
		if (size > 0.0f && radius > 0.0f && detail > 0.0f)
		{
			if (size > 180.0f) size = 180.0f;

			var lightning = SgtComponentPool<SgtLightning>.Pop("Lightning", parent);
